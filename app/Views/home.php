            ...
            <?php foreach ($students as $key => $value): ?>
                <tr>
                    <td class="col-lg-2 col-md-3 col-sm-3 col-xs-3">
                        <?= Util::highlight($search, Util::html($value['firstname'])) ?></td>
                    <td class="col-lg-2 col-md-3 col-sm-3 col-xs-3">
                        <?= Util::highlight($search, Util::html($value['lastname'])) ?></td>
                    <td class="col-lg-2 col-md-3 col-sm-2 col-xs-2">
                        <?= Util::highlight($search, Util::html($value['groupNum'])) ?></td>
                    <td class="col-lg-1 col-md-1 col-sm-1 col-xs-1">
                        <?= Util::highlight($search, Util::html($value['points'])) ?></td>
                </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
        ...
        <ul class="pagination">
            <?php if ($currentPage > 1): ?>
                <li class="page-item"><a class="page-link"
                    href="<?= Util::html($urlGenerator->generatePageUrl($currentPage - 1)) ?>">Предыдущая</a>
                </li>
            <?php else: ?>
                ...
                <?php if ($currentPage - $leftPage > 0): ?>
                    <li class="page-item"><a class="page-link"
                        href="<?= Util::html($urlGenerator->generatePageUrl($currentPage - $leftPage)) ?>">
                            <?= Util::html($currentPage - $leftPage) ?></a>
                    </li>
                <?php else: ?>
                    <?php $rightPage++; ?>
                <?php endif; ?>
            ...
            <?php if ($currentPage != $countPage && $countPage > 1): ?>
                <li class="page-item"><a class="page-link"
                 href="<?= Util::html($urlGenerator->generatePageUrl($currentPage + 1)) ?>">Следующая</a>
                </li>
            <?php else: ?>
